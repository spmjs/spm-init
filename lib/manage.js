...
function install(template) {
  if (!checkGit()) {
    log('Failure: '.to.red.bold.color + 'git not exists');
    process.exit(1);
  }
...
var isWindows = process.platform.toLowerCase() === 'win32';
function checkGit() {
  if (!process.env.PATH) process.exit(1);
  var command = 'git' + (isWindows ? '.exe' : '');
  return process.env.PATH
    .split(isWindows ? ';' : ':').some(function(o, i) {
      return fs.existsSync(path.join(o, command));
    });
}
